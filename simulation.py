...
def generateVehicles(lane_index, image_path, direction):
    try:
        ...
        for bbox in results[0].boxes:
            ...
            # Ensure class_id is valid before using it
            if class_id < len(vehicleTypes):
                vehicle_type = vehicleTypes[class_id]
                lane_number = lane_index
                direction_number = lane_index
                Vehicle(lane_number, vehicle_type, direction_number, directionNumbers[direction_number])
            else:
                print(f"Invalid class ID detected: {class_id}")
...
def generate_vehicles():
    while True:
        for lane_index in range(4):
            generateVehicles(lane_index,lane_images[lane_index], direction_numbers[lane_index])
        time.sleep(2)
direction_numbers = ['right', 'down', 'left', 'up']
lane_images = ['/home/ranjith/Pictures/lane.png', '/home/ranjith/Pictures/lane.png', '/home/ranjith/Pictures/lane.png', '/home/ranjith/Pictures/lane.png']
class Main:
    ...
    thread2 = threading.Thread(name="generate_vehicles",target=generate_vehicles, args=())    # Generating vehicles
    thread2.daemon = True
    thread2.start()
